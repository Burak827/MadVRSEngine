...
- Docker (API + PostgreSQL): `docker-compose up --build`  
  API: `http://localhost:8080/swagger`, DB: `localhost:5432` (`vendorrisk` / `postgres` / `postgres`).
...
- **Infrastructure (`VendorRisk.Infrastructure`)**: EF Core `VendorDbContext`, `VendorProfileRepository`, `FileRiskFactorMatrixProvider`, `RuleEngineService` (rule-based scorer), `DataSeeder` (loads seed vendors).
...
## Docker Notes
- API listens on `8080`; PostgreSQL exposed on `5432`.
- Environment already preconfigured in `docker-compose.yml` to use Postgres (`DatabaseProvider=Postgres`).

## What’s Next (not implemented)
- Redis caching for high-frequency vendor lookups.
